...
## 2024-05-18 - [Optimization WaiterOrderClient]
**Learning:** Re-renders mapping large arrays (like a menu) and executing an O(N) `.find()` on another list (like a cart) per iteration causes O(N*M) performance bottlenecks in Next.js applications, and synchronously triggering this during an input change blocks the main thread.
**Action:** Use a memoized `Map` keyed by item ID for O(1) lookups during array mapping and use React's `useDeferredValue` hook to defer expensive filtering, keeping typing instantly responsive.
