...
## 2026-09-18 - [Fixing Integration Test Environments]
**Learning:** Integration tests using `process.env` checks for secrets (e.g. `FIREBASE_API_KEY`) might fail in fork PR environments where GitHub Actions injects `***` dummy secrets for unauthorized runs.
**Action:** Verify secrets using robust checks like `.startsWith('AIza')` rather than mere truthiness to gracefully skip integration test suites in restricted environments without causing failures.
