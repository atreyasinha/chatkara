...
## 2026-09-12 - Collapsible Button Needs aria-expanded Attribute
**Learning:** Collapsible sections like the "Add Off-Menu Custom Item" section in WaiterOrderClient need the `aria-expanded` attribute so that screen readers correctly announce whether the panel is expanded or collapsed.
**Action:** When implementing collapsible UI elements, always accompany them with an `aria-expanded={isOpen}` attribute.
