...
## 2026-09-14 - Explicit ARIA Labels for Minimalist Waiter Inputs
**Learning:** Rapid-entry forms designed for staff (like WaiterOrderClient) often omit visible `<label>` tags to maximize screen real estate. While visually clean, this leaves screen readers with only `placeholder` text, which is an accessibility anti-pattern.
**Action:** When designing space-constrained internal tools or POS interfaces without explicit `<label>` tags, always use `aria-label` on `<input>` and `<textarea>` elements to ensure full accessibility for staff using assistive technologies.
