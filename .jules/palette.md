...
## 2025-02-23 - Waiter UI Accessibility
**Learning:** Rapid entry POS systems often rely solely on placeholders for space efficiency, causing accessibility issues for screen readers. Collapsible UI elements in rapid order environments need explicit state.
**Action:** Always complement placeholder-only inputs (like rapid order forms) with explicit `aria-label` attributes and ensure collapsible buttons use `aria-expanded` attributes.
