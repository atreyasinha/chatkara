...
## 2026-09-17 - [CRITICAL] Unauthenticated SSRF in Telegram Webhook
**Vulnerability:** The webhook endpoint `/api/telegram/webhook/route.ts` parsed the JSON payload and used user-supplied data (`update.callback_query.id`) to make external API calls to Telegram (`answerTelegramCallback`) even when the webhook secret was invalid. This created a Server-Side Request Forgery (SSRF) and API spoofing vulnerability, as well as a JSON parsing DoS risk.
**Learning:** Never use unauthenticated user-supplied data to execute backend logic or make external API calls. Authentication must happen first.
**Prevention:** Always verify cryptographically secure tokens in headers before parsing the request body (e.g., before `await request.json()`). Do not fall back to inspecting user-supplied payloads or executing cleanup logic based on them prior to validation.
