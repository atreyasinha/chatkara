...
## 2026-09-08 - SSRF and DoS Prevention in Webhooks
**Vulnerability:** Telegram webhook endpoint processed untrusted JSON and used unverified payload fields (callback_query.id) in outbound requests before verifying the cryptographic webhook secret.
**Learning:** This exposes the application to JSON parsing DoS attacks and SSRF/spoofing vulnerabilities.
**Prevention:** Always verify the cryptographic secret via headers before parsing the request body or executing external API calls with user-supplied data.
